...
(function ($) {
    $.fn.w_registeredAndAcceptedRequestAmountsByTimePeriod = function (options) {

        var widgetArea = this;
        var grid = null;
        var settings = $.extend({
            widget: null,
            getRegisteredAndAcceptedRequestAmountsByTimePeriod: '/api/CoreReports/getRegisteredAndAcceptedRequestAmountsByTimePeriod',
            timePeriod: 2,
            numberFewAgo: 1,
        ...
        this.reBind = function (type, objectID) {
            ...
            $.ajax({
                url: settings.getRegisteredAndAcceptedRequestAmountsByTimePeriod + "?TimePeriod=" + timePeriod + '&start=' + start + '&end=' + end,
                type: 'GET',
                ...
                success: function (data) {
                    ...
                    if (data.length == 0) {
                        widgetArea.find('#registeredAndAcceptedRequestAmountsByTimePeriodChart').html("اطلاعاتی جهت نمایش وجود ندارد");
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            registerData.push(data[i].registeredAmountsSum);
                            accentData.push(data[i].acceptedAmountsSum);
                        }
        ...
        function loadChart(categories, registerData, accentData) {
            var options = {
                series: [{
                    name: 'مبالغ ثبت شده و پایان یافته در چرخه تسهیلات و شروع فرایند مالی ',
                    data: registerData
                }, {
                    name: 'مبالغ تایید شده در حال گذراندن فرایند چرخه تسهیلات',
                    data: accentData
                }],
                chart: {
                    foreColor: '#9ba7b2',
                ...
                title: {
                    text: 'نمودار مبلغ درخواست‌های ثبت و تایید در هرماه ',
                    align: 'right',
                    style: {
                        fontSize: '14px'
                    }
            ...
            var chartArea = widgetArea.find('#registeredAndAcceptedRequestAmountsByTimePeriodChart');
        ...
        function getTemplate() {
            var s = `               
				<div class="card-body">
					<div id="registeredAndAcceptedRequestAmountsByTimePeriodChart"></div>
				</div>
            `;
...
(function ($) {
    $.fn.w_registeredAndAcceptedRequestAmountsByTimePeriodSettings = function (options) {
...
dashboardWidgets["94e9aae8-2695-418f-b71e-a8f707b76ff2"] = {

    createWidget: function (item, w, c) {

        return $(item).w_registeredAndAcceptedRequestAmountsByTimePeriod({ widget: w, configObject: c });
    },
